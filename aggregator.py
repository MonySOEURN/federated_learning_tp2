...
class Aggregator(ABC):
    ...
    Returns:
        ...
        self.sampled_clients_ids = self.rng.sample(range(0, self.n_clients), self.n_clients_per_round)
        ...
        if self.sample_with_replacement:
            ...
            self.sampled_clients_ids = self.rng.choices(range(0, self.n_clients), weights=self.clients_weights, k=self.n_clients_per_round)
        else:
            print("Sampling with NOT replacement")
            self.sampled_clients_ids = self.rng.sample(range(0, self.n_clients), self.n_clients_per_round)
